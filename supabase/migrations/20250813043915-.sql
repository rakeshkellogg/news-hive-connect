...
DROP TRIGGER IF EXISTS update_group_invites_updated_at ON public.group_invites;
...
-- 7) Clean up any overly permissive profiles policies if present (fixing column name)
DO $$
BEGIN
  IF EXISTS (
  ...
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'profiles' AND policyname = 'Users can view profiles'
  ) THEN
    EXECUTE 'DROP POLICY "Users can view profiles" ON public.profiles;';
  END IF;
END $$;
