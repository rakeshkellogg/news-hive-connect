...
-- Only super_admin can manage/view user_roles
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 3) Role-check function (security definer to avoid recursive RLS issues)
...
-- 4) Seed super_admin for the provided email (if the user exists already)
...
-- 5) Audit logs table (privacy-first: minimal fields, omit full row payloads)
...
-- 6) Audit trigger function (SECURITY DEFINER) - store minimal data by default
...
-- 7) Attach audit triggers to core tables
DO $$
BEGIN
  -- posts
  IF NOT EXISTS (
...
-- 8) Super admin bypass policies for all core tables
-- Groups
DO $$
BEGIN
